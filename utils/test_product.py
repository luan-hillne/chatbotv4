...
def recommend_closet(df, origin_CNCT, number = 5):
  fil_df = df[df["origin_CNCT"].str.lower() == origin_CNCT.lower()]
  sorted_df = fil_df.sort_values(by=["distanceMeters"]).head(number)
  
  for index, row in sorted_df.iterrows():
    destination_CNCT = row["destination_CNCT"]
    distance = row["distanceMeters"]/1000
    duration = float(row["duration"].rstrip('s'))/3600
    hours = int(duration)
    minutes = int((duration - hours) * 60)
    duration = f"{hours} hours {minutes} minutes"
    
    print(f"Destination CNCT: {destination_CNCT}, Distance: {distance} km, duration: {duration}")
  # return sorted_df.head(number)

# recommend_closet(df, "hni")
...
print(get_inventory('điều hòa','hni'))